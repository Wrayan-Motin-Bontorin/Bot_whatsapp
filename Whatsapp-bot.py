...
class WhatsappBotGUI:
    def __init__(self, root):
        ...
        self.selected_files = []
        self.selected_db_file = None  # Added initialization for selected_db_file
    ...
    def start_sending(self):
        ...
        if not message:  # Check if message is empty
            tk.messagebox.showerror("Erro", "Por favor, insira uma mensagem antes de iniciar o envio.")
            return
        if not self.selected_db_file:  # Check if db file is selected
            tk.messagebox.showerror("Erro", "Por favor, selecione um arquivo de banco de dados.")
            return
        if not self.selected_files:  # Check if files are selected
            tk.messagebox.showerror("Erro", "Por favor, selecione os arquivos a serem enviados.")
            return
